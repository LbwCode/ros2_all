...
#include "interface_all/action/move_circle.hpp"  // 自定义的圆周运动接口
...
class MoveCircleActionClient : public rclcpp::Node
{
    public:
        ...
        using CustomAction = interface_all::action::MoveCircle;
        ...
        void send_goal(bool enable)
        {
            ...
            {
                RCLCPP_ERROR(this->get_logger(), "Client: Action server not available after waiting 等待动作反馈");
                rclcpp::shutdown();
                return;
            }
            ...
            // 异步方式发送动作的目标
            RCLCPP_INFO(this->get_logger(), "Client: Sending goal 已发送 动作指令");
    ...
    private:
        ...
        void goal_response_callback(GoalHandle::SharedPtr goal_message)
		{
			if (!goal_message)
			{
				RCLCPP_ERROR(this->get_logger(), "Client: Goal was rejected by server 获取 动作状态失败");
				rclcpp::shutdown(); // Shut down client node
			}
			else
			{
				RCLCPP_INFO(this->get_logger(), "Client: Goal accepted by server, waiting for result 开始等待 动作状态反馈");
			}
		}
        ...
        {
            std::stringstream ss;
            ss << "Client: Received feedback: 旋转度数 "<< feedback_message->state;
        ...
        void result_callback(const GoalHandle::WrappedResult & result_message)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Client: Result received: 动作结束 %s", (result_message.result->finish ? "true" : "false"));
            rclcpp::shutdown();         // 关闭客户端节点
        }
};
