...
class MoveCircleActionServer : public rclcpp::Node
{
    ...
    private:
        ...
        {
            RCLCPP_INFO(this->get_logger(), "Server: Received goal request: 收到 正在执行 %d", goal_request->enable);
            (void)uuid;
        ...
        void execute(const std::shared_ptr<GoalHandle> goal_handle_)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Server: Executing goal 动作开始执行");
            ...
            for (int i = 0; (i < 361) && rclcpp::ok(); i=i+30)
            {
                ...
                RCLCPP_INFO(this->get_logger(), "Server: Publish feedback 正在旋转 %d", i);
            ...
            if (rclcpp::ok())
            {
                ...
                RCLCPP_INFO(this->get_logger(), "Server: Goal succeeded 当前动作结束");
            }
        }
};
